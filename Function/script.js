...
/* =========================================================================
   CLASS ROUTINE - Custom Fixed Slots
   ========================================================================= */
...
function renderRoutineGridDesktop() {
  ...
  const slotLabels = [
    { time: "8:15 AM", mins: 8 * 60 + 15 },
    { time: "9:45 AM", mins: 9 * 60 + 45 },
    { time: "11:15 AM", mins: 11 * 60 + 15 },
    { time: "12:45 PM", mins: 12 * 60 + 45 },
...
function renderRoutineMobile() {
  ...
  if (!dayEvents.length) {
    listContainer.innerHTML = emptyStateHtml(
      ...
      `<button class="btn btn-primary" onclick="openRoutineForm()">${icon("plus")}<span>Add class</span></button>`,
    );
    return;
  }
...
function renderAssignments() {
  ...
  if (!state.assignments.length) {
    container.innerHTML = emptyStateHtml(
      ...
      `<button class="btn btn-primary" onclick="openAssignmentForm()">${icon("plus")}<span>Add assignment</span></button>`,
    );
    return;
  }
...
function renderExamsList() {
  ...
  if (!upcoming.length && !past.length) {
    container.innerHTML = emptyStateHtml(
      ...
      `<button class="btn btn-primary" onclick="openExamForm()">${icon("plus")}<span>Add exam</span></button>`,
    );
    return;
  }
  let html = "";
...
function renderAttendanceSubjects() {
  ...
  if (!state.attendance.subjects.length) {
    grid.innerHTML = emptyStateHtml(
      ...
      `<button class="btn btn-primary" onclick="openAttendanceSubjectForm()">${icon("plus")}<span>Add subject</span></button>`,
    );
    return;
  }
...
function renderSemesters() {
  ...
  if (!state.cgpa.semesters.length) {
    container.innerHTML = emptyStateHtml(
      ...
      `<button class="btn btn-primary" onclick="openSemesterForm()">${icon("plus")}<span>Add semester</span></button>`,
    );
    return;
  }
    ...
    <div class="semester-block">
      ...
      <table class="course-table">
        ...
        <tbody>
          ...
          : `<div class="empty-state" style="padding: var(--space-6);"><p>No courses added to this semester yet.</p></div>`
      }
    </div>`;
    })
    .join("");
...
function renderMaterials() {
  ...
  if (!state.materials.length) {
    container.innerHTML = emptyStateHtml(
      ...
      `<button class="btn btn-primary" onclick="openMaterialForm()">${icon("plus")}<span>Add material</span></button>`,
    );
    return;
  }
...
/* =========================================================================
   USER PROFILE (Base64 Database HACK + Name Edit)
   ========================================================================= */
  ...
  try {
    ...
    const modalHtml = `
      ...
      <div id="profile-view-mode">
          <div style="position: relative; margin: -20px -20px 20px -20px; text-align: center;">
              <div style="height: 150px; ${coverStyle} border-radius: 8px 8px 0 0;"></div>
              ...
              <h2 style="margin: 0; font-size: 22px; color: var(--text-primary);">${esc(userName)}</h2>
          ...
          <div class="dashboard-grid" style="grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 25px;">
              <div style="background: var(--surface-hover); padding: 15px; border-radius: 8px; text-align: center;">
                  <div style="font-size: 20px; font-weight: bold; color: var(--pine);">${cgpa != null ? cgpa.toFixed(2) : "N/A"}</div>
                  <div style="font-size: 11px; color: var(--text-tertiary); text-transform: uppercase; margin-top:4px;">Current CGPA</div>
              </div>
              <div style="background: var(--surface-hover); padding: 15px; border-radius: 8px; text-align: center;">
                  <div style="font-size: 20px; font-weight: bold; color: var(--pine);">${attendance != null ? attendance + "%" : "N/A"}</div>
                  <div style="font-size: 11px; color: var(--text-tertiary); text-transform: uppercase; margin-top:4px;">Attendance</div>
              </div>
              <div style="background: var(--surface-hover); padding: 15px; border-radius: 8px; text-align: center;">
                  <div style="font-size: 20px; font-weight: bold; color: var(--pine);">${totalCredits}</div>
                  <div style="font-size: 11px; color: var(--text-tertiary); text-transform: uppercase; margin-top:4px;">Credits</div>
              </div>
          </div>

          <div style="margin-bottom: 20px;">
              <h4 style="margin: 0 0 10px 0; font-size: 13px; color: var(--text-tertiary); text-transform: uppercase;">Digital Badges</h4>
      ...
      <div id="profile-edit-mode" class="hidden">
          <h3 style="margin-top:0;">Edit Profile & Socials</h3>
          
          <div class="form-group">
            <label>Full Name</label>
            <input id="edit-name" class="input" placeholder="e.g. John Doe" value="${esc(userName)}">
          </div>

          <div class="form-row">
            <div class="form-group">
                <label>Change Avatar Picture</label>
                <input type="file" id="avatar-upload" class="input" accept="image/*">
            </div>
            <div class="form-group">
                <label>Change Cover Photo</label>
                <input type="file" id="cover-upload" class="input" accept="image/*">
            </div>
          </div>

          <div class="form-group">
            <label>GitHub Username / Link</label>
            <input id="edit-github" class="input" placeholder="e.g. TheRudraBro" value="${esc(github)}">
          </div>
          <div class="form-group">
            <label>LinkedIn Link</label>
            <input id="edit-linkedin" class="input" placeholder="e.g. linkedin.com/in/yourname" value="${esc(linkedin)}">
          </div>
          <div class="form-group">
            <label>Portfolio Website</label>
            <input id="edit-portfolio" class="input" placeholder="e.g. rudra.dev" value="${esc(portfolio)}">
          </div>
...
window.openRoutineForm = openRoutineForm;
window.openAssignmentForm = openAssignmentForm;
window.openExamForm = openExamForm;
window.openAttendanceSubjectForm = openAttendanceSubjectForm;
window.openSemesterForm = openSemesterForm;
