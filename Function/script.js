...
function openRoutineForm(id) {
  ...
  if (isEdit)
    document
      ...
      .addEventListener("click", () => handleDeleteRoutine(id));
}
...
function openAssignmentForm(id) {
  ...
  if (isEdit)
    document
      ...
      .addEventListener("click", () => handleDeleteAssignment(id));
}
...
function openExamForm(id) {
  ...
  if (isEdit)
    document
      .getElementById("examDeleteBtn")
      .addEventListener("click", () => handleDeleteExam(id));
}
    ...
    .map((cls) => {
      ...
      const existingRecord = subj
        ? state.attendance.records.find(
            (r) => r.subjectId === subj.id && r.date === todayDate,
          )
        : null;
...
function openCourseForm(semId, courseId) {
  ...
  if (course)
    document.getElementById("courseDeleteBtn").addEventListener("click", () => {
      closeModal();
...
function openMaterialForm(id) {
  ...
  if (isEdit)
    document
      ...
      .addEventListener("click", () => handleDeleteMaterial(id));
}
...
// BUG FIX: HTML er vitore onclick jinish gula jeno thik moto kaj kore module mode e
...
// Firebase Auth check kore tarpor asol kaj shuru kora hobe
document.addEventListener("DOMContentLoaded", () => {
  onAuthStateChanged(auth, (user) => {
    if (user && user.emailVerified) {
      currentUserUid = user.uid; // Logged in user er id ta set kora holo
      init();
    } else {
      // User jodi na thake ba verified na thake, index.html e pathiye dibe
